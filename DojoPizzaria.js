var order = "";
var text = document.getElementById("textScroll");
...
function userInput(choice){
    ...
    buttonPressCounter++;
    console.log(order);
    console.log("button press counter is ", buttonPressCounter);
    textUpdate(order);
}
...
function textUpdate(pizza){
    let cheeses = "";
    let toppings = "";
    
    for (let i = 0; i < pizza.cheeses.length; i++){
        cheeses += pizza.cheeses[i] + ", ";
    }
    ...
    text.innerHTML = "" + pizza.crustType + ", " + pizza.sauceType + ", " + cheeses + toppings;
}
